      ...
      <v-row class="pa-1 ma-1 ">
        <v-row>
          ...
          <v-col><v-sheet class="ma-5">
              <router-link to="/firstpage"> <v-btn large plain class="mx-3"
                  style="background-color: #a88a5e; color: #1d1d1d; transition: box-shadow 0.3s;border-radius: 999px;">
                  <p class="font-weight-bold">+</p>
                </v-btn></router-link>
            </v-sheet>
...
export default {
  ...
  async created() {
    ...
    this.fetchPosts(token); // Fetch posts when the component is created
  },
  methods: {
    async fetchPosts(token) {
      this.posts = await pAPI.getAllPlace(token);
    },
  },
};
</script>